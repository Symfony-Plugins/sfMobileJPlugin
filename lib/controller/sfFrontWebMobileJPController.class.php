...
class sfFrontWebMobileJPController extends sfFrontWebController
{
    public function genUrl($parameters = array(), $absolute = false)
    {
       ...
       // for DoCoMo
        ...
        if ($agent->isDoCoMo() && sfConfig::get('sf_mobile_jp_is_required_uid', false)) {
            $url .= ((strpos($url, '?') === false) ? '?' : '&') . (sfConfig::get('sf_mobile_jp_use_guid_for_docomo', false) ? 'guid=ON' : 'uid=NULLGWDOCOMO');
        }
