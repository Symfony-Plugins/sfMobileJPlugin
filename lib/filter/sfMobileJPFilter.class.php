        ...
        if ($agent->isDoCoMo() || $agent->isSoftBank() || $agent->isEZweb()) {
            ...
            if (sfConfig::get('sf_mobile_jp_is_required_uid', false)) {
                ...
                if ($agent->isDoCoMo()) {
                    ...
                    $isRedirect  = false;
                    $queryString = null;
                    if (sfConfig::get('sf_moible_jp_use_guid_for_docomo', false)) {
                        $logger->debug('{sfMobileJPFilter} required imodeID(guid)');
                        $uid = $agent->getUID();
                        if (empty($uid)) $uid = null;
                        if (is_null($uid) && !$request->hasParameter('guid')) {
                            $isRedirect = true;
                            $queryString = 'guid=ON';
                        }
                    } else {
                        $logger->debug('{sfMobileJPFilter} required uid');
                        $uid = $request->getParameter('uid', null);
                        if (is_null($uid) && !$request->hasParameter('uid')) {
                            $isRedirect = true;
                            $queryString = 'uid=NULLGWDOCOMO';
                        }
            ...
            if ($agent->isDoCoMo()) {
                $response->setContentType('Content-type: application/xhtml+xml');
            }
